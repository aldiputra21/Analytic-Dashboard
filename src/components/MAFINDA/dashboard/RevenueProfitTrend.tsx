...
interface RevenueProfitTrendProps {
...
export const RevenueProfitTrend: React.FC<RevenueProfitTrendProps> = ({
  data,
  isLoading,
}) => {
  ...
  const yearlyData = React.useMemo(() => {
    ...
    const byYear = new Map<number, { revenue: number; netProfit: number }>();
    
    data.forEach(d => {
      const year = parseInt(d.period.split('-')[0], 10);
      const existing = byYear.get(year) ?? { revenue: 0, netProfit: 0 };
      byYear.set(year, {
        revenue: existing.revenue + d.revenue,
        netProfit: existing.netProfit + d.netProfit
      });
    });
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm h-full min-h-[350px] flex flex-col">
      <div className="mb-4">
        <h3 className="text-sm font-bold text-slate-800">{mt.revenueProfit}</h3>
