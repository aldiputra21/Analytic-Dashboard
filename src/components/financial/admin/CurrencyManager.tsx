...
import {
  ...
  ChevronLeft, ChevronRight, DollarSign, X,
...
import { currencyI18n } from '../../../i18n/currency';
...
export const CurrencyManager: React.FC = () => {
  ...
  const t = currencyI18n[language];
  const common = commonsI18n[language];

  // Validation Schema
  const currencySchema = z.object({
    code: z.string().min(1, t.validation.codeRequired).min(3, t.validation.codeMin),
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/currencies?${query.toString()}`);
  ...
  const openModal = (mode: 'create' | 'edit' | 'view', item?: Currency) => {
    setModalMode(mode);
    if (item) {
      setEditingId(item.id);
      setFormData({ code: item.code, label: item.label, status: item.status });
    } else {
      setEditingId(null);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const validation = currencySchema.safeParse({
        ...formData,
  ...
  const handleDelete = async (id: string) => {
    setIsDeleting(true);
    try {
      const res = await apiFetch(`/api/currencies/${id}`, { method: 'DELETE' });
      if (res.ok) {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">{t.tableHead.label}</th>
                ...
                {loading ? (
                  ...
                  data.map((currency, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-800 text-sm font-semibold">{currency.label}</td>
                      <td className="px-6 py-4">
                        <div className={cn(
                          ...
                          <div className={cn('w-1.5 h-1.5 rounded-full', currency.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400')} />
                          {currency.status === 'active' ? t.status.active : t.status.inactive}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => openModal('view', currency)}
        ...
        {isModalOpen && (
          ...
          >
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                  {t.modal.label} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
