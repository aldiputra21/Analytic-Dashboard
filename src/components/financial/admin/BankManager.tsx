...
export const BankManager: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/banks?${query.toString()}`);
  ...
  const openModal = (mode: 'create' | 'edit' | 'view', item?: Bank) => {
    setModalMode(mode);
    if (item) {
      ...
      setEditingId(null);
      setFormData({ code: '', name: '', swiftCode: '', status: 'active' });
    }
    setIsModalOpen(true);
  };
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">{t.tableHead.name}</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">{t.tableHead.swiftCode}</th>
                ...
                {loading ? (
                  ...
                  data.map((bank, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-800 text-sm font-semibold">{bank.name}</td>
                      <td className="px-6 py-4 text-slate-500 text-xs font-mono">{bank.swiftCode ?? '—'}</td>
                      <td className="px-6 py-4">
                        <div className={cn(
                          ...
                          <div className={cn('w-1.5 h-1.5 rounded-full', bank.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400')} />
                          {bank.status === 'active' ? t.status.active : t.status.inactive}
                        </div>
                      </td>
        ...
        {isModalOpen && (
          ...
          >
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                  {t.modal.swiftCodeOptional}
                </label>
                <input
                  type="text"
                  value={formData.swiftCode}
                  onChange={(e) => setFormData(p => ({ ...p, swiftCode: e.target.value.toUpperCase() }))}
                  disabled={isReadOnly}
                  maxLength={20}
                  placeholder="CENAIDJA"
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-sm font-mono uppercase disabled:opacity-70 disabled:cursor-not-allowed"
                />
              </div>
