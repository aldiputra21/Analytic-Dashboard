...
import {
  ...
  ChevronLeft, ChevronRight, Tag, X,
...
import { corporateSectorI18n } from '../../../i18n/corporate-sector';
...
export const CorporateSectorManager: React.FC = () => {
  const { hasPermission, language } = useAuth();
  const t = corporateSectorI18n[language];
  ...
  const [data, setData] = useState<CorporateSector[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/corporate-sectors?${query.toString()}`);
  ...
  const openModal = (mode: 'create' | 'edit' | 'view', item?: CorporateSector) => {
    setModalMode(mode);
    if (item) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const validation = sectorSchema.safeParse({
        ...formData,
      ...
      const url = editingId ? `/api/corporate-sectors/${editingId}` : '/api/corporate-sectors';
  ...
  const handleDelete = async (id: string) => {
    setIsDeleting(true);
    try {
      const res = await apiFetch(`/api/corporate-sectors/${id}`, { method: 'DELETE' });
      if (res.ok) {
                ...
                {loading ? (
                  ...
                  data.map((sector, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-800 text-sm font-semibold">{sector.labelId}</td>
                      <td className="px-6 py-4 text-slate-500 text-sm">{sector.labelEn}</td>
                      <td className="px-6 py-4">
                        <div className={cn(
                          ...
                          <div className={cn('w-1.5 h-1.5 rounded-full', sector.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400')} />
                          {sector.status === 'active' ? t.status.active : t.status.inactive}
                        </div>
                      </td>
