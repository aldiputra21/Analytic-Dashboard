...
const EXPORT_OPTIONS: ExportOption[] = [
  { format: 'csv', label: 'CSV', icon: Table, description: { id: 'Nilai yang dipisahkan koma', en: 'Comma-separated values' } },
  { format: 'excel', label: 'Excel', icon: Table, description: { id: 'Microsoft Excel (.xlsx)', en: 'Microsoft Excel (.xlsx)' } },
  { format: 'pdf', label: 'PDF', icon: FileText, description: { id: 'PDF dengan branding', en: 'PDF with branding' } },
  { format: 'png', label: 'PNG', icon: Image, description: { id: 'Screenshot dashboard', en: 'Dashboard screenshot' } },
];
...
export const ExportButton: React.FC<ExportButtonProps> = ({
  ...
  const handleExport = async (format: ExportFormat) => {
    ...
    try {
      if (format === 'png') {
        ...
        if (chartRef?.current) {
          try {
            ...
            alert(language === 'id' ? 'Ekspor PNG membutuhkan html2canvas. Silakan gunakan format lain.' : 'PNG export requires html2canvas. Please use another format.');
          }
        } else {
          alert(language === 'id' ? 'Tidak ada elemen chart untuk diekspor PNG.' : 'No chart element available for PNG export.');
        }
        return;
      }
      ...
      alert(`${common.export} failed: ${err.message}`);
    } finally {
  ...
  return (
    <div className={cn('relative', className)} ref={dropdownRef}>
      ...
      >
        ...
        {isExporting ? `${common.exporting} ${exportingFormat?.toUpperCase()}...` : common.export}
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">{option.description[language]}</p>
                </div>
              </button>
            );
          })}
