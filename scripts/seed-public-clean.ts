// scripts/seed-public-clean.ts — Clean Public schema data for testing
// Run with: npx tsx scripts/seed-public-clean.ts
...
async function main() {
  ...
  const uploadTemplateConfigs = [
    ...
    // 1. Balance Sheet (Neraca)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'corporate_id', 'period', 'cash_and_bank', 'accounts_receivable',
          'work_in_progress', 'inventory', 'prepaid_expenses', 'land',
          'building', 'equipment', 'other_fixed_assets', 'accounts_payable',
          'bank_loan_current', 'other_current_liabilities', 'bank_loan_long_term',
          'other_long_term_liabilities', 'shareholder_loan', 'capital',
          'earnings_after_tax', 'retained_earnings', 'dividends', 'notes',
        ],
      },
    ...
    // 2. Income Statement (Laba Rugi)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'corporate_id', 'period', 'revenue', 'cogs', 'operating_expenses',
          'interest_expense', 'tax_expense', 'other_income', 'other_expense', 'notes',
        ],
      },
    ...
    // 3. Income Statement Projection (Proyeksi Laba Rugi) — flat format
    {
      key: 'upload_template_income_statement_projection',
      value: {
        startRecord: 4,
        columnOrder: [
          'header_ref', 'department_id', 'project_id', 'fiscal_year', 'month',
          'account_code', 'account_name', 'amount', 'notes',
        ],
      },
    ...
    // 4. Weekly Cash Flow (Arus Kas Mingguan)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'corporate_id', 'entity_type', 'entity_id', 'period', 'week',
          'operating_cash_in', 'operating_cash_out', 'investing_cash_in',
          'investing_cash_out', 'financing_cash_in', 'financing_cash_out', 'notes',
        ],
      },
    ...
    // 5. Realization (Realisasi)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'entity_type', 'department_id', 'project_id', 'transaction_date',
          'category', 'cost_center_id', 'amount', 'notes',
        ],
      },
    ...
    // 6. Cash Flow Projection (Proyeksi Arus Kas) — wide format
    // Row 3 = month names (merged 2 cols each), Row 4 = cash_in/cash_out sub-headers, Row 5+ = data
    {
      key: 'upload_template_cash_flow_projection',
      value: {
        startRecord: 5,
        columnOrder: [
          'corporate', 'fiscal_year', 'initial_balance', 'notes',
          'jan_cash_in', 'jan_cash_out', 'feb_cash_in', 'feb_cash_out',
          'mar_cash_in', 'mar_cash_out', 'apr_cash_in', 'apr_cash_out',
          'may_cash_in', 'may_cash_out', 'jun_cash_in', 'jun_cash_out',
          'jul_cash_in', 'jul_cash_out', 'aug_cash_in', 'aug_cash_out',
          'sep_cash_in', 'sep_cash_out', 'oct_cash_in', 'oct_cash_out',
          'nov_cash_in', 'nov_cash_out', 'dec_cash_in', 'dec_cash_out',
        ],
      },
    ...
    // 7. Bank Loan (Pinjaman Bank)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'corporate_id', 'bank_id', 'credit_type', 'amount', 'start_date',
          'tenor', 'interest_type', 'interest_rate', 'alert_min_days',
        ],
      },
    ...
    // 8. Corporate (Perusahaan)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'name', 'code', 'industry', 'currency', 'fiscal_year_start_month', 'tax_rate',
        ],
      },
    ...
    // 9. Department (Departemen)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'corporate_id', 'name', 'code', 'description', 'head_name',
        ],
      },
    ...
    // 10. Cost Center
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'corporate_id', 'name', 'code', 'category', 'description',
        ],
      },
    ...
    // 11. Project (Proyek)
    {
      ...
      value: {
        startRecord: 4,
        columnOrder: [
          'department_id', 'name', 'code', 'description', 'start_date', 'end_date', 'status',
        ],
      },
  ...
  const uploadWorkflowDefs = [
    ...
    // ── Master Data Modules (4) ──────────────────────────────
    {
      ...
      makerRole: corporateAdminRoleId, subjectFields: uploadSubjectFields, isActive: false,
      steps: [
        { stepOrder: 1, stepType: 'approval', requiredRole: financeManagerRoleId, isActive: true },
        { stepOrder: 2, stepType: 'approval', requiredRole: financeLeaderRoleId, isActive: true },
      ],
...
0 as januari_estimated, 0 as januari_actual, 0 as februari_estimated, 0 as februari_actual, 0 as maret_estimated, 0 as maret_actual, 0 as april_estimated, 0 as april_actual, 0 as mei_estimated, 0 as mei_actual, 0 as juni_estimated, 0 as juni_actual, 0 as juli_estimated, 0 as juli_actual, 0 as agustus_estimated, 0 as agustus_actual, 0 as september_estimated, 0 as september_actual, 0 as oktober_estimated, 0 as oktober_actual, 0 as november_estimated, 0 as november_actual, 0 as desember_estimated, 0 as desember_actual`
  ...
  console.log('\n🎉 Clean Public schema seeding complete!');
